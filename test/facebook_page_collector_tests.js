...
describe('Facebook Page Collector collects data from pages or user profiles', function() {
	...
	it('should specify fields to retrieve from api', function() {
		var collector = new FacebookPageCollector({cursor: {id: 101}});
	...
	it('should accept q / since / until / limit parameters and convert to timestamp', function() {
		var collector = new FacebookPageCollector({q: 'foo', cursor: {id: 101, oldest: Date.parse('2015-01-01 00:00:00'), newest: Date.parse('2015-01-30 00:00:00'), limit: 50}})
	...
	it('should read comments and retrieve comment cursors for each post', function(done) {
		...
		collector.readSource(function(result) {
			...
			assert.ok(Object.equal(result.contents[0].commentCursor, {parentCreationDate: Date.parse('2015-04-28T15:39:28+0000'), newest: Date.parse('2015-04-28T15:59:28+0000'), oldest: Date.parse('2015-04-28T15:40:28+0000'), after: '__after', before: '__before'}));
			done();
		})
	})
})