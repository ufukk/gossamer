...
describe('Twitter collector collect data using search API', function() {
	...
	it('should set search parameters according to cursor positions', function() {
		var collector = new TwitterCollector({keywords: keywords, cursor:{forwardPosition: 10, backwardPosition: 0}})
		var params = collector.cursorParameters()
		assert.equal(params['since_id'], 10)
		assert.equal(typeof(params['max_id']), 'undefined')

		var collector = new TwitterCollector({keywords: keywords, cursor:{forwardPosition: 0, backwardPosition: 10}})
		var params = collector.cursorParameters()
		assert.equal(params['max_id'], 10)
		assert.equal(typeof(params['since_id']), 'undefined')
		
		var collector = new TwitterCollector({keywords: keywords, cursor:{forwardPosition: 0, backwardPosition: 0}})
		var params = collector.cursorParameters()
		assert.equal(typeof(params['max_id']), 'undefined')
		assert.equal(typeof(params['since_id']), 'undefined')
	})
	...
	it('should join keywords with `OR` to build search query', function() {
		var collector = new TwitterCollector({keywords: ['#foo1', 'foo2'], cursor:{forwardPosition: 10, backwardPosition: 0}})
